...
class Login_Machnine(QMainWindow, QWidget, form_class):       # QMainWindow : PyQt5에서 윈도우 생성시 필요한 함수
    ...
    def __init__(self, *args, **kwargs):                      # Main class의 self를 초기화 한다.
        ...
        self.Auto_start.clicked.connect(self.auto)
    ...
    def Load_code(self):
        ...
        if os.path.exists("main/Selected_code.txt"):
            f = open("main/Selected_code.txt", "r", encoding="utf8")
    ...
    def Save_selected_code(self):
        ...
        for row in range(self.buylast.rowCount()):
            ...
            f = open("main/Selected_code.txt", "a",encoding="utf8")  # "a" 달아 쓴다. "w" 덮어 쓴다. files라느 파이썬 페키지 볼더를 만든다.
    ...
    def delet_code(self):
        ...
        if os.path.exists("main/Selected_code.txt"):
            os.remove("main/Selected_code.txt")
